...
## BF16 分支状态（2026-09-08）

本分支另含纯 BF16 适配 `3ebc8e1e`、普通 GMM 专家列表回退修复 `44abd7d1`，以及实验性复制输入优化 `0433817f`。后者需配套 CANN `e731ea414`，通过 `mega_moe_replicated_input=true` 显式开启，默认关闭；仅适用于已检查的 A2、TP2/EP2、DP1、H2048/I512/E256/top-k8 布局。

188 项 CPU 测试通过；新算子通过两 rank、四种路由分布的 BF16 GMM 参考容差检查，以及每组 5 次同步、20 次异步重复稳定性检查。均匀路由下旧/新最终输出相对 L2 差异约 0.002847，归约分组改变，不是逐位无损。

Qwen3.6-35B-A3B 纯 BF16 整模型首轮通过语义检查和同版本重复稳定性，但一个 2K 随机 token 请求的生成文本与旧版不同，旧版关闭 MegaMoe 的输出也与新版不同。原精度门槛据此停止验收，设备及端口已清理，未执行 V6 整模型性能测试。该实验尚无正的端到端收益结论；下文既有 W8A8 性能与 GSM8K 结果不适用于此 BF16 实验。

已验证的 BF16 V3 算子基线、测试方法与复现输入见配套 [CANN BF16 报告](https://github.com/Liuchenbing-2026/ops-transformer/blob/codex/megamoe-bf16-e2e-20260907/mc2/mega_moe/docs/bf16_ep2_reproduction.md)。
