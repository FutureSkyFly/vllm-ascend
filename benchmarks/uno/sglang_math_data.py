...
def prepare_benchmark_data(name: str, *, output_dir: Path) -> Path:
    ...
    if output.is_file():
        ...
        if count == benchmark.expected_rows:
            return output
        raise ValueError(f"{output} has {count} rows; expected {benchmark.expected_rows}")

    records = BUILDERS[name]()
    if len(records) != benchmark.expected_rows:
        raise ValueError(f"Expected {benchmark.expected_rows} rows for {name}, got {len(records)}")
