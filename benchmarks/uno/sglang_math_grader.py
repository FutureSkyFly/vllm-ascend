...
# Vendored from SGLang 554f817948c26e8e9c8338b4a33e94a609d6f0fb, benchmark/uno.
# Original preparation/grading semantics retained for cross-engine evaluation.
...
def _answer_candidates(text: str) -> list[Any]:
    boxed = extract_last_boxed_content(text)
    candidates = (
        [_parse_boxed_content(boxed), normalize_answer_text(boxed), boxed] if boxed else [_parse_unboxed_answer(text)]
    )
    unique = []
    seen = set()
...
def score_math(
    ...
    for index, row in enumerate(graded):
        ...
        samples.extend((sample_index + offset, bool(value)) for offset, value in enumerate(row["correct"]))

    per_problem = [[value for _, value in sorted(samples)] for samples in by_source.values() if samples]
        ...
        summary.update(
            {
                ...
                f"pass_at_{samples_per_problem}": _get_accuracy([any(samples) for samples in per_problem]),
                f"num_pass_at_{samples_per_problem}": sum(any(samples) for samples in per_problem),
            }
        )
